...
model {
  ...
  z_month ~ normal(0, 1);
  z_year ~ normal(0, 1);
  phi ~ exponential(1);  // Overdispersion prior

  // Likelihood
  for (n in 1:N)
    fatalities[n] ~ neg_binomial_2_log(
      alpha + day_effect[day[n]] + month_effect[month[n]] + year_effect[year[n]], 
      phi);
}
generated quantities {
  ...
  for (n in 1:N) {
    fatalities_pred[n] = neg_binomial_2_log_rng(
      alpha + day_effect[day[n]] + month_effect[month[n]] + year_effect[year[n]], 
      phi);
  }
  for (d in 1:N_day) {
